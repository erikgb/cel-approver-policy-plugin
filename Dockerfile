...
# Build
RUN go build -o cert-manager-csi-approver-policy-plugin main.go
...
WORKDIR /
USER 1001
COPY --from=builder /workspace/cert-manager-csi-approver-policy-plugin /usr/bin/cert-manager-approver-policy
